...
def smart_page_html(summary: dict[str, Any], *, admin: bool = False, message: str = "") -> str:
    ...
    *{{box-sizing:border-box}}body{{margin:0;background:#f5f5f7;color:#1d1d1f;font-family:-apple-system,"Microsoft JhengHei",sans-serif}}
    ...
    </style></head><body><div class="wrap"><header><div><h1>曜核智控 2.0</h1><p>{escape(summary['month'])} 智慧分析與目標管理</p></div><div>{admin_link}</div></header>{notice}
...
def jarvis_layout(body: str, active: str, title: str, boot: bool = False) -> str:
    ...
    boot_html = '<div id="boot" class="boot"><div class="boot-stage"><div class="boot-halo halo-a"></div><div class="boot-halo halo-b"></div><div class="boot-ticks"></div><div class="boot-orbit"><div class="boot-core" aria-label="系統核心"><i></i><b></b></div></div><div class="boot-needle"></div></div><div class="boot-kicker">財務核心啟動</div><h1>金</h1><p id="boot-line">正在點火...</p><div class="boot-track"><i></i></div><div class="boot-status"><span>動力核心</span><span>資料鏈路</span><span>AI 引擎</span></div></div>' if boot else ""
    css = r'''
    ...
    *{box-sizing:border-box}html{background:var(--bg)}body{margin:0;min-height:100vh;color:var(--text);font-family:var(--ui);background:radial-gradient(circle at 83% 0%,rgba(92,65,255,.19),transparent 35%),radial-gradient(circle at 8% 30%,rgba(0,210,255,.11),transparent 28%),linear-gradient(145deg,var(--bg),var(--bg2) 58%,#030710);overflow-x:hidden}body:before{content:"";position:fixed;inset:0;pointer-events:none;opacity:.16;background-image:linear-gradient(45deg,rgba(255,255,255,.018) 25%,transparent 25%,transparent 75%,rgba(255,255,255,.018) 75%),linear-gradient(45deg,rgba(255,255,255,.018) 25%,transparent 25%,transparent 75%,rgba(255,255,255,.018) 75%);background-size:8px 8px;background-position:0 0,4px 4px}a{color:inherit;text-decoration:none}
    .app-shell{min-height:100vh;display:grid;grid-template-columns:250px minmax(0,1fr)}.sidebar{position:sticky;top:0;height:100vh;padding:22px 16px;border-right:1px solid var(--line);background:rgba(4,10,19,.93);backdrop-filter:blur(22px);z-index:30;display:flex;flex-direction:column}.brand-block{display:flex;align-items:center;gap:12px;padding:4px 6px 25px}.brand-flag{position:relative;width:72px;height:58px;display:grid;place-items:center;filter:drop-shadow(0 0 10px rgba(255,200,87,.72));animation:flagBreathe 1.65s ease-in-out infinite}.brand-flag:before{content:"";position:absolute;left:4px;top:0;width:4px;height:58px;border-radius:4px;background:linear-gradient(#fff1a8,#d89218 42%,#7c4300);box-shadow:0 0 10px #ffc857,0 0 24px rgba(255,159,67,.75)}.brand-flag:after{content:"";position:absolute;left:8px;top:6px;width:58px;height:42px;background:linear-gradient(135deg,#6e3900 0%,#f2b72d 18%,#ffe39a 48%,#ca7b09 76%,#5b2600 100%);clip-path:polygon(0 0,100% 0,82% 50%,100% 100%,0 100%);border:1px solid rgba(255,225,135,.98);box-shadow:inset 0 0 15px rgba(255,255,255,.28),inset -8px 0 13px rgba(138,20,8,.42),0 0 15px rgba(255,58,29,.85),0 0 42px rgba(255,190,49,.85)}.brand-flag span{position:relative;z-index:2;margin-left:7px;font:700 27px var(--title);color:#070707;-webkit-text-stroke:.7px rgba(255,226,132,.75);text-shadow:0 1px 0 rgba(255,255,255,.2),0 0 9px rgba(255,200,87,.72)}.brand-text b,.mobile-brand b{font-family:var(--title);font-size:24px;color:#ffe7a0;text-shadow:0 0 10px rgba(255,200,87,.75)}.brand-text small,.mobile-brand small{display:none}.nav-stack{display:grid;gap:8px}.nav-item{position:relative;display:flex;align-items:center;gap:13px;padding:12px 14px;border-radius:14px;color:#9fb2c3;transition:.25s;overflow:hidden}.nav-item:before{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(54,214,255,.14),transparent);opacity:0}.nav-item:hover,.nav-item.active{color:#fff;transform:translateX(3px);border:1px solid rgba(75,211,255,.18)}.nav-item:hover:before,.nav-item.active:before{opacity:1}.nav-item.active:after{content:"";position:absolute;left:0;top:20%;bottom:20%;width:3px;border-radius:5px;background:var(--accent);box-shadow:0 0 16px var(--accent)}.nav-icon{width:26px;text-align:center;font-size:19px;z-index:1}.nav-item span:last-child{z-index:1;font-size:14px}.side-status{margin-top:auto;border:1px solid var(--line);background:rgba(10,20,35,.66);border-radius:17px;padding:14px}.online{display:flex;align-items:center;gap:8px;color:#bfffe8;font-size:12px}.pulse-dot{width:8px;height:8px;border-radius:50%;background:var(--green);box-shadow:0 0 13px var(--green);animation:pulse 1.8s ease-in-out infinite}.side-status small{display:block;color:var(--muted);margin-top:7px;line-height:1.5}
    ...
    .content-wrap{min-height:100vh;overflow:visible}main{display:block;min-height:calc(100vh - 78px);overflow:visible}.dashboard-grid,.stats-grid,.content-grid,.gauge-grid{position:relative;z-index:1;width:100%}.dashboard-grid+ .dashboard-grid{clear:both}.hero-grid{align-items:stretch}.hero-card,.ai-card,.stat-card,.section-card,.gauge-card{visibility:visible!important}.dashboard-end-marker{height:1px;clear:both}
    .boot{position:fixed;z-index:1000;inset:0;display:grid;place-content:center;justify-items:center;background:radial-gradient(circle at 50% 42%,rgba(27,55,92,.92),#02050b 62%);transition:.7s cubic-bezier(.22,.8,.2,1);overflow:hidden}.boot:before{content:"";position:absolute;inset:-30%;background:conic-gradient(from 0deg,transparent,rgba(54,214,255,.08),transparent 18%,rgba(123,97,255,.1),transparent 43%);animation:bootSky 9s linear infinite}.boot.hide{opacity:0;visibility:hidden;transform:scale(1.045);filter:blur(8px)}.boot-stage{width:210px;height:210px;position:relative;display:grid;place-items:center}.boot-halo{position:absolute;border-radius:50%;border:1px solid rgba(110,225,255,.22);box-shadow:0 0 26px rgba(54,214,255,.12),inset 0 0 26px rgba(123,97,255,.08)}.halo-a{inset:0;animation:spin 7s linear infinite}.halo-b{inset:20px;border-style:dashed;border-color:rgba(145,113,255,.32);animation:spin 3.8s linear reverse infinite}.boot-ticks{position:absolute;inset:8px;border-radius:50%;background:repeating-conic-gradient(from -120deg,rgba(123,231,255,.85) 0 1deg,transparent 1deg 8deg);mask:radial-gradient(circle,transparent 0 72%,#000 73% 100%);opacity:.55;animation:tickPulse 1.4s ease-in-out infinite}.boot-orbit{width:132px;height:132px;border-radius:50%;display:grid;place-items:center;position:relative;background:radial-gradient(circle,rgba(15,35,59,.42),rgba(4,10,19,.12));box-shadow:0 0 50px rgba(54,214,255,.16);animation:orbitFloat 2.4s ease-in-out infinite}.boot-orbit:before,.boot-orbit:after{content:"";position:absolute;border-radius:50%;border:3px solid transparent}.boot-orbit:before{inset:0;border-top-color:var(--accent);border-right-color:rgba(54,214,255,.34);animation:spin 1.2s linear infinite}.boot-orbit:after{inset:16px;border-bottom-color:var(--purple);border-left-color:rgba(123,97,255,.38);animation:spin .82s linear reverse infinite}.boot-core{width:74px;height:74px;border-radius:50%;display:grid;place-items:center;background:radial-gradient(circle at 38% 32%,#fff 0 3%,#8ff0ff 9%,#317edb 32%,#4d2dad 52%,#071326 72%);box-shadow:0 0 18px rgba(54,214,255,.9),0 0 48px rgba(123,97,255,.55),0 0 86px rgba(54,214,255,.22),inset 0 0 20px rgba(255,255,255,.22);animation:coreIgnite 1.35s ease-in-out infinite;position:relative}.boot-core:before{content:"";position:absolute;inset:10px;border-radius:50%;border:1px solid rgba(255,255,255,.46);box-shadow:inset 0 0 13px rgba(255,255,255,.2)}.boot-core i,.boot-core b{position:absolute;display:block;border-radius:50%}.boot-core i{width:11px;height:11px;background:#fff;box-shadow:0 0 13px #fff,0 0 28px var(--accent)}.boot-core b{width:42px;height:42px;border:1px solid rgba(171,243,255,.55);animation:spin 2.2s linear infinite}.boot-needle{position:absolute;width:4px;height:88px;left:50%;top:50%;transform-origin:50% 100%;transform:translate(-50%,-100%) rotate(-115deg);background:linear-gradient(to top,transparent 0 8%,#ff4f75 30%,#fff 100%);clip-path:polygon(42% 100%,58% 100%,100% 0,0 0);filter:drop-shadow(0 0 8px #ff4f75);animation:needleSweep 2.15s cubic-bezier(.22,.8,.2,1) forwards}.boot-kicker{margin-top:12px;font-size:10px;letter-spacing:5px;color:#75ddff;text-transform:uppercase}.boot h1{font:34px var(--title);letter-spacing:5px;margin:10px 0 7px;text-shadow:0 0 22px rgba(54,214,255,.35)}.boot p{color:#adbed0;font-size:12px;letter-spacing:1px}.boot-track{width:min(300px,72vw);height:3px;background:rgba(255,255,255,.07);overflow:hidden;margin-top:16px;border-radius:99px;box-shadow:0 0 14px rgba(54,214,255,.1)}.boot-track i{display:block;height:100%;width:0;background:linear-gradient(90deg,var(--accent),var(--purple),var(--green),var(--gold));box-shadow:0 0 14px var(--accent);animation:bootbar 2.45s cubic-bezier(.2,.8,.2,1) forwards}.boot-status{width:min(320px,78vw);display:flex;justify-content:space-between;margin-top:13px;color:#5f7890;font-size:9px;letter-spacing:1px}.boot-status span{position:relative;padding-left:10px}.boot-status span:before{content:"";position:absolute;left:0;top:3px;width:5px;height:5px;border-radius:50%;background:var(--green);box-shadow:0 0 7px var(--green);animation:pulse 1.5s ease-in-out infinite}
    @keyframes pulse{50%{opacity:.38;transform:scale(.78)}}@keyframes breathe{50%{filter:brightness(1.25);transform:scale(1.018)}}@keyframes scan{0%,72%{left:-80%}92%,100%{left:150%}}@keyframes reveal{to{opacity:1;transform:none}}@keyframes revealSafe{from{opacity:.01;transform:translateY(14px) scale(.992)}to{opacity:1;transform:none}}@keyframes draw{to{stroke-dashoffset:0}}@keyframes dotIn{to{opacity:1}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes bootbar{to{width:100%}}@keyframes bootSky{to{transform:rotate(360deg)}}@keyframes tickPulse{50%{opacity:.95;filter:brightness(1.45)}}@keyframes orbitFloat{50%{transform:translateY(-4px) scale(1.025)}}@keyframes coreIgnite{50%{filter:brightness(1.35);transform:scale(1.07)}}@keyframes needleSweep{0%{transform:translate(-50%,-100%) rotate(-115deg)}55%{transform:translate(-50%,-100%) rotate(72deg)}72%{transform:translate(-50%,-100%) rotate(38deg)}100%{transform:translate(-50%,-100%) rotate(58deg)}}@keyframes ambientShift{0%,100%{transform:translate3d(-2%,-2%,0) scale(1)}50%{transform:translate3d(2%,1%,0) scale(1.08)}}@keyframes borderPulse{50%{box-shadow:0 22px 65px rgba(0,0,0,.48),0 0 26px color-mix(in srgb,var(--accent) 8%,transparent),inset 0 1px rgba(255,255,255,.04)}}@keyframes navGlow{50%{box-shadow:inset 0 0 18px rgba(54,214,255,.13),0 0 15px rgba(123,97,255,.1)}}@keyframes flagBreathe{0%,100%{filter:drop-shadow(0 0 10px rgba(255,198,55,.82)) drop-shadow(0 0 16px rgba(255,38,20,.58));transform:scale(1) rotate(-.4deg)}50%{filter:drop-shadow(0 0 25px rgba(255,239,159,1)) drop-shadow(0 0 48px rgba(255,50,22,1));transform:scale(1.075) rotate(.7deg)}}@keyframes heroReturn{0%{opacity:0;transform:translateY(18px) scale(.985)}100%{opacity:1;transform:none}}
    .boot h1{color:#ffe7a0;text-shadow:0 0 7px #fff,0 0 20px #ffc857,0 0 42px #ff7b00;font-size:48px}.boot-core{box-shadow:0 0 20px rgba(54,214,255,.95),0 0 52px rgba(123,97,255,.68),0 0 95px rgba(255,200,87,.2),inset 0 0 20px rgba(255,255,255,.22)}
    ...
    body:after{content:"";position:fixed;inset:-20%;pointer-events:none;z-index:-1;background:radial-gradient(circle at 25% 28%,rgba(54,214,255,.09),transparent 24%),radial-gradient(circle at 72% 64%,rgba(123,97,255,.1),transparent 27%),radial-gradient(circle at 48% 90%,rgba(40,229,166,.05),transparent 24%);animation:ambientShift 12s ease-in-out infinite}
    .panel{transition:transform .34s cubic-bezier(.2,.8,.2,1),border-color .34s,box-shadow .34s;animation:borderPulse 6s ease-in-out infinite}.panel:hover{transform:translateY(-5px) scale(1.006);border-color:rgba(96,224,255,.34);box-shadow:0 26px 72px rgba(0,0,0,.55),0 0 30px rgba(54,214,255,.09),inset 0 1px rgba(255,255,255,.05)}
    .stat-card:nth-child(2n){animation-delay:.6s}.stat-card:nth-child(3n){animation-delay:1.1s}.gauge-card:nth-child(2n){animation-delay:.8s}.gauge-card:nth-child(3n){animation-delay:1.4s}
    .brand-flag{will-change:filter,transform}.hero-grid>.hero-card{display:grid!important;visibility:visible!important;opacity:1;transform:none;animation:heroReturn .72s cubic-bezier(.2,.8,.2,1) both!important;animation-delay:0s!important}
    .icon-btn{transition:.25s}.icon-btn:hover{transform:translateY(-2px) rotate(-3deg);border-color:rgba(110,228,255,.4);box-shadow:0 0 20px rgba(54,214,255,.16)}
    .speed-value{animation:meterWake 1.7s cubic-bezier(.2,.8,.2,1) both}.chart-area{filter:drop-shadow(0 0 11px rgba(54,214,255,.16))}.chart-dot{animation:dotIn .5s ease forwards,pulse 2.4s ease-in-out infinite}.ai-message div{transition:.28s}.ai-message div:hover{transform:translateX(5px);background:rgba(133,91,255,.11);box-shadow:0 0 18px rgba(123,97,255,.08)}
    .mobile-nav{animation:navGlow 4s ease-in-out infinite}.mobile-nav-item.active span{filter:drop-shadow(0 0 8px var(--accent));animation:orbitFloat 2.2s ease-in-out infinite}
